...
Renderer::Renderer()
{
	m_bone_animation = new Bone_Animation();
}
...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	// Create nanogui gui for camera
	nanogui::FormHelper *gui_1 = new nanogui::FormHelper(m_nanogui_screen);
	nanogui::ref<nanogui::Window> nanoguiWindow_2 = gui_1->addWindow(Eigen::Vector2i(0, 0), "Nanogui control bar_1");
	...
	// Create nanogui gui for rotation
	//screen->setPosition(Eigen::Vector2i(-width/2 + 200, -height/2 + 300));

	gui_1->addGroup("Bone Rotation");

	// Widgets for bone 1
	static const int numBones = 3; // Adjust this based on the number of bones in your system
	static std::vector<nanogui::Widget*> boneWidgets;
	...
	for (int i = 1; i < numBones + 1; i++) {
		...
		auto rotate_x_widget = gui_1->addVariable("Rotate X " + boneName, m_bone_animation->rotation_degree_vector[i][2]);
		auto rotate_y_widget = gui_1->addVariable("Rotate Y " + boneName, m_bone_animation->rotation_degree_vector[i][0]);
		auto rotate_z_widget = gui_1->addVariable("Rotate Z " + boneName, m_bone_animation->rotation_degree_vector[i][1]);

		boneWidgets.push_back(rotate_x_widget);
		boneWidgets.push_back(rotate_y_widget);
		boneWidgets.push_back(rotate_z_widget);
	}
	...
	gui_1->addButton("Reset Bone Rotation", [this]() {
		for (int i = 1; i <= numBones; i++) {
			std::string boneName = "Bone " + std::to_string(i);
			auto rotate_x_widget = static_cast<nanogui::FloatBox<float>*>(boneWidgets[(i) * 3]);
			auto rotate_y_widget = static_cast<nanogui::FloatBox<float>*>(boneWidgets[(i) * 3 + 1]);
			auto rotate_z_widget = static_cast<nanogui::FloatBox<float>*>(boneWidgets[(i) * 3 + 2]);

			// Set the values of the sliders to 0
			rotate_x_widget->setValue(0.0f);
			rotate_y_widget->setValue(0.0f);
			rotate_z_widget->setValue(0.0f);

		}
		// Call the Rotation function here to apply the reset immediately
		m_bone_animation->reset();
		m_bone_animation->Rotation();
		});

	m_nanogui_screen->setVisible(true);
	m_nanogui_screen->performLayout();

	// Create nanogui gui for target

	gui_1->addGroup("Target");

		is_moving = gui_1->addVariable("Bone Move", m_bone_animation->isMoving);
		auto target_x_widget = gui_1->addVariable("target X:", m_bone_animation->target[0]);
		auto target_y_widget = gui_1->addVariable("target Y:", m_bone_animation->target[1]);
		auto target_z_widget = gui_1->addVariable("target Z:", m_bone_animation->target[2]);

		target_x_widget->setEditable(true);
		target_y_widget->setEditable(true);
		target_z_widget->setEditable(true);

		target_x_widget->setCallback([this](float value) {
			m_bone_animation->target[0] = value;
			});

		target_y_widget->setCallback([this](float value) {
			m_bone_animation->target[1] = value;
			});

		target_z_widget->setCallback([this](float value) {
			m_bone_animation->target[2] = value;
			});

		auto current_x_widget = gui_1->addVariable("current X:", m_bone_animation->e[0]);
		auto current_y_widget = gui_1->addVariable("current Y:", m_bone_animation->e[1]);
		auto current_z_widget = gui_1->addVariable("current Z:", m_bone_animation->e[2]);

		boneWidgets.push_back(target_x_widget);
		boneWidgets.push_back(target_y_widget);
		boneWidgets.push_back(target_z_widget);
